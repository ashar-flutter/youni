// material
export 'package:flutter/material.dart';
export 'package:flutter/gestures.dart';



// getX -
export 'package:get/get.dart' hide ScreenType;

// responsive sizer
export 'package:responsive_sizer/responsive_sizer.dart';

// screens
export 'package:youni/splash.dart';
export 'package:youni/account_subscription.dart';
export 'package:youni/auth/login.dart';
export 'package:youni/auth/register.dart';
export 'package:youni/auth/reset_password.dart';
export 'package:youni/auth/reset_password_sent.dart';
export 'package:youni/auth/new_password.dart';


//google fonts
export 'package:google_fonts/google_fonts.dart';

//config
export 'package:youni/config/app_colors.dart';
export 'package:youni/config/app_image.dart';

// widgets
export 'package:youni/widgets/custom_text.dart';
export 'package:youni/widgets/custom_scaffold.dart';
export 'package:youni/widgets/app_button.dart';
export 'package:youni/widgets/on_tap.dart';
export 'package:youni/widgets/app_text_field.dart';
export 'package:youni/widgets/auth_header.dart';
export 'package:youni/widgets/social_row.dart';
export 'package:youni/widgets/auth_footer_section.dart';

